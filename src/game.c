...
void game_run_frame(GameData *d, Graphics *g, Audio *a, Input *i,
    ...
    case GAME_STATE_STARTUP:
        ...
        if (d->startSelectedFlash > 2.0f) {
            ...
            game_request_open_modal(d, &m_size, &textStartingPoint, menuTitle,
    ...
    if (state_stack_peek(&d->gameState) == GAME_STATE_EDITOR) {
        ...
        graphics_draw_box(g, d->scene.spawnPoint.x, d->scene.spawnPoint.y, 32,
                          32, COLOR_BLUE, 100);
    }
    /* Player Rendering */
    ...
    if (d->foundItem > 0.0f) {
        graphics_draw_tile(g, 0, d->interactionData.item.tile, b->x, b->y - 40,
                           32, 32);
    ...
    if (state_stack_peek(&d->gameState) == GAME_STATE_STARTUP) {
        if (d->fadeIn < 6.5f) {
            ...
            graphics_draw_box(g, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, COLOR_BLACK,
            ...
            if (d->startSelected &&
                ...
                graphics_draw_text(g, SCREEN_WIDTH / 2 - 32,
                                   SCREEN_HEIGHT / 2 + 96, 48, "Press Start");
