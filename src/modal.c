...
static void modal_draw(Graphics *g, const Modal *m, float timeStep) {
    graphics_draw_box(g, m->dim.x, m->dim.y, m->dim.w, m->dim.h, COLOR_BLUE,
                      255);
...
static void tile_picker_draw(Graphics *g, const TilePicker *p) {
    ...
    // maxFrameTilesHorizontal - 1: col) * w); int y = (int)
    // ((float)(maxFrameTilesVertical <= row ? maxFrameTilesVertical - 1 : row)
    // * h); drawBox(x, y, (int)w, (int)h, Color::WHITE, 100);
}
