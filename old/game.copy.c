...
static void
draw_text(GBuffer* g, int x, int y, int w, int h, const char* text);

static void
draw_tiles(GBuffer* g, int r, int c, int *tiles);

static void
draw_tile(GBuffer* g, int x, int y, int tile);
			...
			switch (in->instructions[i].action) {
				...
				case IACTION: {
					...
					for (j = 0; j < s->entityCount; j++) {
						...
						if (point_in_body(e->x, e->y, e->w, e->h, s->x, s->y)) {
								e->vis = 1;
						}
					}
					break;
...
static void run(GameState* s, float ts) {
	...
	if (s->x > SCREEN_WIDTH + 32) {
		s->x = -32;
	}

	if (s->y < -32) {
		s->y = SCREEN_HEIGHT + 32;
	}

	if (s->y > SCREEN_HEIGHT + 32) {
		s->y = -32;
	}
}
...
static void* galloc(MBuffer* m, int s) {
	...
	retVal = (unsigned char*)m->m + m->head;
	m->head += s;
	return retVal;
}
...
void game_run(GBuffer* g, IBuffer *in, MBuffer *m, float ts) {
	...
	if (!m->head) {
		...
		/* Remove this later */
		s->entityCount = 0;
		s->entities[s->entityCount].x = 32;
		s->entities[s->entityCount].y = 32;
		s->entities[s->entityCount].w = 32;
		s->entities[s->entityCount].h = 32;
		s->entities[s->entityCount].vis = 0;
		strcpy(s->entities[s->entityCount].text, "Hello!");
		s->entityCount += 1;
		s->entities[s->entityCount].x = 64;
		s->entities[s->entityCount].y = 64;
		s->entities[s->entityCount].w = 64;
		s->entities[s->entityCount].h = 64;
		s->entities[s->entityCount].vis = 0;
		strcpy(s->entities[s->entityCount].text, "Goodbye!");
		s->entityCount += 1;
		/* Remove this later */
	} else {
	...
	draw_tiles(g, 13, 19, s->background);
	draw_text(g, s->x, s->y, s->w, s->h, "A");
	draw_tile(g, 0, 0, 55);
	{
			int i;
			Entity* e;
			for (i = 0; i < s->entityCount; i++) {
					e = &s->entities[i];
					if (e->vis) {
							draw_text(g, e->x, e->y, 128, 32, e->text);
					}
			}
	}
