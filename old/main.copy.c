...
void process_graphics(SDL_Renderer* r, SDL_Texture* tileMap, TTF_Font* f, GBuffer* g) {
	int i;
	SDL_Rect out;
	SDL_Surface *s;
	SDL_Texture *t;
	...
	for (i = 0; i < g->idx; i++) {
		switch (g->instructions[i].type) {
			...
			case GTILE: {
				...
				out.x = x, out.y = y, out.w = 32, out.h = 32;
